...
function activate(context) {
  const disposable = vscode.commands.registerCommand(
    ...
    function () {
      ...
      while (funcNameStart >= 0 && /[\w.\$]/.test(text[funcNameStart]))
        funcNameStart--;
      ...
      if (depth !== 0) {
        vscode.window.showErrorMessage(
          "Couldn't find matching closing parenthesis."
        );
        return;
      }
      ...
      if (paramText.trim() === "") {
        vscode.window.showInformationMessage(
          "Empty parameters detected, skipping formatting."
        );
        return;
      }
      ...
      function formatArgument(arg, depth) {
        ...
        if (open !== -1 && close !== -1 && open < close) {
          ...
          const formattedInner = innerArgs
            .map((a) => formatArgument(a, depth + 1))
            .join(",\n");
